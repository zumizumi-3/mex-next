...
function latestPhaseHistory(
  ...
  const entries = raw
    .filter(isRecord)
    .filter((entry) => matchesCadence(entry, cadence));
...
function phaseTimestamp(entry: Record<string, unknown>): number {
  for (const key of ['ended_at', 'completed_at', 'updated_at', 'created_at', 'date', 'period_end']) {
...
function fillThreeOptions(options: string[]): string[] {
  const fallback = ['前回方針を維持する', '反応が良かった軸を強める', '今週の制約に合わせて方針を変える'];
  const out = [...options];
