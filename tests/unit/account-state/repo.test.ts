...
import { assertSafeId, InvalidContentIdError } from '../../../src/account-state/repo.js';
...
describe('AccountRepo', () => {
  ...
  it('withState で transaction 的に state を更新', async () => {
    ...
    const persisted = JSON.parse(await readFile(join(workDir, 'state.json'), 'utf-8')) as {
      skip_dates: string[];
    };
    expect(persisted.skip_dates).toEqual(['2026-05-02', '2026-05-03']);
  });

  it('git_sync called after withState mutation', async () => {
    const syncMutation = vi.fn(async () => ({
      committed: true,
      pushed: true,
      commitHash: 'abc',
    }));
    const repoWithGit = new AccountRepo(workDir, {
      gitSync: { syncMutation } as unknown as GitSync,
    });

    await repoWithGit.withState(async (state) => ({
      state: { ...state, skip_dates: ['2026-05-04'] },
      result: undefined,
    }));

    expect(syncMutation).toHaveBeenCalledTimes(1);
    expect(syncMutation).toHaveBeenCalledWith('chore(state): mutation');
  });
  ...
  it('並列 withState で counter increment がロストしない', async () => {
    ...
    const final = JSON.parse(await readFile(join(workDir, 'state.json'), 'utf-8')) as {
      _counter: number;
    };
