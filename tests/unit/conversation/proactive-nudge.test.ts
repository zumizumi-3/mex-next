...
function makeBridge(text = JSON.stringify({ summary: '先週は専門性を出す方針でした。', options: ['維持', '実績を強める', '対象を変える'] })): LlmProvider {
  return {
...
describe('emitNudge', () => {
  ...
  it('weekly_phase_review は直近 weekly phase_history を LLM に渡して Discord に投稿する', async () => {
    ...
    const result = await emitNudge({ repo, bridge, poster, logger: makeLogger() }, 'weekly_phase_review');
  ...
  it('monthly_phase_review は monthly cadence の nudge を投稿する', async () => {
    ...
    const bridge = makeBridge(JSON.stringify({ summary: '先月は採用向けでした。', options: ['維持', '発信量を増やす', '読者を変える'] }));
    const poster = makePoster();

    const result = await emitNudge({ repo, bridge, poster, logger: makeLogger() }, 'monthly_phase_review');
  ...
  it('stale_target_review は 1 週間候補がない target の見直しを投稿し、LLM は呼ばない', async () => {
    ...
    const result = await emitNudge({ repo, bridge, poster, logger: makeLogger() }, 'stale_target_review');
  ...
  it('unanswered_phase_followup は 3 日以上止まった phase questionnaire を促す', async () => {
    ...
    const result = await emitNudge({ repo, bridge: makeBridge(), poster, logger: makeLogger() }, 'unanswered_phase_followup');
