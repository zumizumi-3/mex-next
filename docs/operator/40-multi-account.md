...
> **Legacy design note.** Current multi-account operations are documented in
> [31-multi-account.md](./31-multi-account.md). Per-account timer/service units
> now use `mex-{name}-{ACCOUNT_ID}.service` / `.timer` generated by
> `scripts/install-systemd-units.sh`; the template-unit examples below are
