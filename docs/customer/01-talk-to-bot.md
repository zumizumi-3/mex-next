...
bot には「予約見せて」「ニュース見せて」「全部取り消して」「@tanaka_san を外して」のように、そのまま頼めます。件数や対象が必要な操作は bot が確認してから進め、曖昧な要求は聞き返します。
...
取り消し / 今すぐ投稿 / 自動運用切替 など「あと戻りしにくい操作」は、必ず **はい / いいえ** の確認が一度出ます。件数や対象が分かる時は、bot が先に数を出します。

```text
あなた: 全部取り消して
bot:    過去 5 件 + 今日 1 件 = 計 6 件を取り消します。実行しますか?
あなた: はい
bot:    ✅ 6 件を取り消しました
```

- `はい` と返すまで何も実行されません
- `いいえ` と返すと「キャンセルしました」で閉じます
- 表示系 (一覧 / 状態確認) は確認なしで即返答

bot は自然な日本語をそのまま受け取ります。対象が曖昧な時は「過去分も含めて全部ですか、今日だけですか?」のように聞き返します。
...
sequenceDiagram
    ...
    Bot-->>You: 「6:18 の予約 1 件を取り消します。実行しますか?」
    You->>Bot: 「はい」
...
bot から質問された時は、そのまま短く答えて大丈夫です。

```text
bot:    過去分も含めて全部ですか、今日だけですか?
あなた: 今日だけ
bot:    今日の予約 1 件を取り消します。実行しますか?
```
...
処理が遅い / 結果がいらなくなった場合、`@bot キャンセル` で進行中の作業を止められます。完了済みの予約は対象外で、未完了の生成や検討中の処理だけが止まります。
