...
`@tanaka_san` でも `tanaka_san` でも同じものとして扱われます。intent-router の `normalizeHandle` が `@` を剥がし、英数 + `_` 以外を落とします。
...
target からのリプ / 引用は reply フローで個別 thread が立ちます (target だから優先する、ではなく risk classifier が判定)。
